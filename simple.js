...
`.trim()
    ...
    * @returns 
    */
    NanoBotz.sendGroupV4Invite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', options = {}) => {
    ...
    * @returns 
    */
    NanoBotz.cMod = (jid, message, text = '', sender = NanoBotz.user.jid, options = {}) => {
        ...
        copy.key.fromMe = areJidsSameUser(sender, NanoBotz.user.id) || false
     ...
     * @param {Object} options
     */
    NanoBotz.copyNForward = async (jid, message, forwardingScore = true, options = {}) => {
        let m = generateForwardMessageContent(message, !!forwardingScore)
        let mtype = Object.keys(m)[0]
        if (forwardingScore && typeof forwardingScore == 'number' && forwardingScore > 1) m[mtype].contextInfo.forwardingScore += forwardingScore
        m = generateWAMessageFromContent(jid, m, { ...options, userJid: NanoBotz.user.id })
        await NanoBotz.relayMessage(jid, m.message, { messageId: m.key.id, additionalAttributes: { ...options } })
        return m
    }
    
    NanoBotz.loadMessage = NanoBotz.loadMessage || (async (messageID) => {
        return Object.entries(NanoBotz.chats)
            .filter(([_, { messages }]) => typeof messages === 'object')
            .find(([_, { messages }]) => Object.entries(messages)
                .find(([k, v]) => (k === messageID || v.key?.id === messageID)))
     ...
     */
    NanoBotz.downloadM = async (m, type, saveToFile) => {
        ...
        if (saveToFile) var { filename } = await NanoBotz.getFile(buffer, true)
     ...
     */
    NanoBotz.chatRead = async (jid, participant = NanoBotz.user.jid, messageID) => {
        return await NanoBotz.sendReadReceipt(jid, participant, [messageID])
    }
    
    NanoBotz.sendStimg = async (jid, path, quoted, options = {}) => {
        let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await fetch(path)).buffer() : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        let buffer
        if (options && (options.packname || options.author)) {
            buffer = await writeExifImg(buff, options)
    ...
    NanoBotz.sendStvid = async (jid, path, quoted, options = {}) => {
    ...
    NanoBotz.getName = (jid = '', withoutContact = false) => {
        jid = NanoBotz.decodeJid(jid)
        withoutContact = this.withoutContact || withoutContact
        let v
        if (jid.endsWith('@g.us')) return new Promise(async (resolve) => {
            v = NanoBotz.chats[jid] || {}
            if (!(v.name || v.subject)) v = await NanoBotz.groupMetadata(jid) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = jid === '0@s.whatsapp.net' ? {
            jid,
            vname: 'WhatsApp'
        } : areJidsSameUser(jid, NanoBotz.user.id) ?
            NanoBotz.user :
            (NanoBotz.chats[jid] || {})
     ...
     */
     NanoBotz.processMessageStubType = async(m) => {
    /**
     * to process MessageStubType
     * @param {import('@adiwajshing/baileys').proto.WebMessageInfo} m 
     */
    if (!m.messageStubType) return
        const chat = NanoBotz.decodeJid(m.key.remoteJid || m.message?.senderKeyDistributionMessage?.groupId || '')
    if (!chat || chat === 'status@broadcast') return
        const emitGroupUpdate = (update) => {
            NanoBotz.ev.emit('groups.update', [{ id: chat, ...update }])
        }
        ...
        if (!isGroup) return
        let chats = NanoBotz.chats[chat]
        if (!chats) chats = NanoBotz.chats[chat] = { id: chat }
        chats.isChats = true
        const metadata = await NanoBotz.groupMetadata(chat).catch(_ => null)
        if (!metadata) return
        chats.subject = metadata.subject
        chats.metadata = metadata
    }
    NanoBotz.insertAllGroup = async() => {
        const groups = await NanoBotz.groupFetchAllParticipating().catch(_ => null) || {}
        for (const group in groups) NanoBotz.chats[group] = { ...(NanoBotz.chats[group] || {}), id: group, subject: groups[group].subject, isChats: true, metadata: groups[group] }
            return NanoBotz.chats
    }
    
    /*NanoBotz.processMessageStubType = async (m) => {
        if (!m.messageStubType) return
        const mtype = Object.keys(m.message || {})[0]
        const chat = NanoBotz.decodeJid(m.key.remoteJid || m.message[mtype] && m.message[mtype].groupId || '')
        const isGroup = chat.endsWith('@g.us')
        if (!isGroup) return
        let chats = NanoBotz.chats[chat]
        if (!chats) chats = NanoBotz.chats[chat] = { id: chat }
    ...
    if (!m) return
        if (!Array.isArray(m)) m = [m]
            for (const message of m) {
                ...
                if (!message) continue
                    if (message.messageStubType && message.messageStubType != WAMessageStubType.CIPHERTEXT) NanoBotz.processMessageStubType(message).catch(console.error)
                        const _mtype = Object.keys(message.message || {})
                    const mtype = (!['senderKeyDistributionMessage', 'messageContextInfo'].includes(_mtype[0]) && _mtype[0]) ||
                    (_mtype.length >= 3 && _mtype[1] !== 'messageContextInfo' && _mtype[1]) ||
                    _mtype[_mtype.length - 1]
                    const chat = NanoBotz.decodeJid(message.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
                    if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
                    /**
                     * @type {import('@adiwajshing/baileys').proto.IContextInfo}
                     */
                    let context = message.message[mtype].contextInfo
                    let participant = NanoBotz.decodeJid(context.participant)
                    const remoteJid = NanoBotz.decodeJid(context.remoteJid || participant)
                    /**
                    ...
                    if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                        ...
                        if (isGroup && !participant) participant = remoteJid
                            const qM = {
                                key: {
                                    remoteJid,
                                    fromMe: areJidsSameUser(NanoBotz.user.jid, remoteJid),
                                    id: context.stanzaId,
                                    participant,
                                },
                            ...
                            let qChats = NanoBotz.chats[participant]
                            if (!qChats) qChats = NanoBotz.chats[participant] = { id: participant, isChats: !isGroup }
                            ...
                            let chats = NanoBotz.chats[chat]
                            if (!chats) {
                                if (isGroup) await NanoBotz.insertAllGroup().catch(console.error)
                                    chats = NanoBotz.chats[chat] = { id: chat, isChats: true, ...(NanoBotz.chats[chat] || {}) }
                            }
                            let metadata, sender
                            if (isGroup) {
                                if (!chats.subject || !chats.metadata) {
                                    metadata = await NanoBotz.groupMetadata(chat).catch(_ => ({})) || {}
                                    if (!chats.subject) chats.subject = metadata.subject || ''
                                    if (!chats.metadata) chats.metadata = metadata
                                }
                            sender = NanoBotz.decodeJid(message.key?.fromMe && NanoBotz.user.id || message.participant || message.key?.participant || chat || '')
                            if (sender !== chat) {
                                let chats = NanoBotz.chats[sender]
                                if (!chats) chats = NanoBotz.chats[sender] = { id: sender }
                    ...
                    if (!chats.messages) chats.messages = {}
                        const fromMe = message.key.fromMe || areJidsSameUser(sender || chat, NanoBotz.user.id)
...
NanoBotz.sendPoll = async (jid, name = '', optiPoll, options) => {
    ...
    return NanoBotz.relayMessage(jid, { pollCreationMessage: pollMessage }, { ...options });
};
...
NanoBotz.setBio = async (status) => {
        return await NanoBotz.query({
            tag: 'iq',
            attrs: {
                to: 's.whatsapp.net',
                type: 'set',
    ...
    /*NanoBotz.pushMessage = async (m) => {
        ...
        for (const message of m) {
            try {
                // if (!(message instanceof proto.WebMessageInfo)) continue // https://github.com/adiwajshing/Baileys/pull/696/commits/6a2cb5a4139d8eb0a75c4c4ea7ed52adc0aec20f
                if (!message) continue
                if (message.messageStubType) NanoBotz.processMessageStubType(message).catch(console.error)
                let mtype = Object.keys(message.message || {})
                mtype = mtype[mtype[0] === 'messageContextInfo' && mtype.length == 2 ? 1 : 0]
                const chat = NanoBotz.decodeJid(message.key.remoteJid || message.message[mtype] && message.message[mtype].groupId || '')
                const isGroup = chat.endsWith('@g.us')
                let chats = NanoBotz.chats[chat]
                if (!chats) {
                    if (isGroup) {
                    ...
                    chats = NanoBotz.chats[chat] = { id: chat, ...(NanoBotz.chats[chat] || {}) }
                }
                let metadata, sender
                if (isGroup) {
                    if (!chats.subject || !chats.metadata) {
                    ...
                    sender = NanoBotz.decodeJid(message.fromMe && NanoBotz.user.id || message.participant || message.key.participant || chat || '')
                    if (sender !== chat) {
                        let chats = NanoBotz.chats[sender]
                        if (!chats) chats = NanoBotz.chats[sender] = { id: sender }
                        if (!chats.name) chats.name = message.pushName || chats.name || ''
                ...
                chats.isChats = true
                const fromMe = message.key.fromMe || areJidsSameUser(chat, NanoBotz.user.id)
                if (!chats.messages) chats.messages = {}
     ...
     * @returns 
     */
    NanoBotz.getBuffer = async (url, options) => {
        try {
            options ? options : {}
     ...
     * @param {Object} m
     */
    NanoBotz.serializeM = (m) => {
        return exports.smsg(NanoBotz , m)
    }

    Object.defineProperty(NanoBotz, 'name', {
        value: 'WASocket',
        configurable: true,
    })
    return NanoBotz 
 ...
 */
 exports.smsg = (NanoBotz, m, hasParent) => {
    if (!m) return m
    ...
    if (m.key) {
        ...
        m.chat = NanoBotz.decodeJid(m.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
        m.isGroup = m.chat.endsWith('@g.us')
        m.sender = NanoBotz.decodeJid(m.key.fromMe && NanoBotz.user.id || m.participant || m.key.participant || m.chat || '')
        m.fromMe = m.key.fromMe || areJidsSameUser(m.sender, NanoBotz.user.id)
    }
    if (m.message) {
        ...
        if (m.mtype == 'protocolMessage' && m.msg.key) {
            ...
            m.msg.key.fromMe = NanoBotz.decodeJid(m.msg.key.participant) === NanoBotz.decodeJid(NanoBotz.user.id)
            if (!m.msg.key.fromMe && m.msg.key.remoteJid === NanoBotz.decodeJid(NanoBotz.user.id)) m.msg.key.remoteJid = m.sender
        }
        ...
        if (m.quoted) {
            ...
            m.quoted.chat = NanoBotz.decodeJid(m.msg.contextInfo.remoteJid || m.chat || m.sender)
            m.quoted.isBaileys = m.quoted.id && m.quoted.id.length === 16 || false
            m.quoted.sender = NanoBotz.decodeJid(m.msg.contextInfo.participant)
            m.quoted.fromMe = m.quoted.sender === NanoBotz.user.jid
            m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.contentText || ''
            m.quoted.name = NanoBotz.getName(m.quoted.sender)
            ...
            m.getQuotedObj = m.getQuotedMessage = async () => {
                ...
                let q = M.fromObject(await NanoBotz.loadMessage(m.quoted.id) || vM)
                return exports.smsg(NanoBotz, q)
            }
            if (m.quoted.url || m.quoted.directPath) m.quoted.download = (saveToFile = false) => NanoBotz.downloadM(m.quoted, m.quoted.mtype.replace(/message/i, ''), saveToFile)
...
/*exports.smsg = (conn, m, hasParent) => {
    ...
    if (m.key) {
        ...
        m.chat = NanoBotz.decodeJid(m.key.remoteJid || m.message[mtype] && m.message[mtype].groupId || '')
        m.isGroup = m.chat.endsWith('@g.us')
        m.sender = NanoBotz.decodeJid(m.fromMe && NanoBotz.user.id || m.participant || m.key.participant || m.chat || '')
        m.fromMe = m.key.fromMe || areJidsSameUser(m.sender, NanoBotz.user.id)
    }
    if (m.message) {
        let mtype = Object.keys(m.message)
        ...
        if (m.quoted) {
            ...
            m.getQuotedObj = m.getQuotedMessage = () => {
                ...
                let q = M.fromObject(((NanoBotz.chats[m.quoted.chat] || {}).messages || {})[m.quoted.id])
             ...
             * @param {Object} options
             */
            m.quoted.reply = (text, chatId, options) => NanoBotz.reply(chatId ? chatId : m.chat, text, vM, options)

            /**
             * Copy quoted message
             */
            m.quoted.copy = () => exports.smsg(NanoBotz, M.fromObject(M.toObject(vM)))
            ...
            */
            m.quoted.forward = (jid, forceForward = false) => NanoBotz.forwardMessage(jid, vM, forceForward)
             ...
             * @param {Object} options
            */
            m.quoted.copyNForward = (jid, forceForward = true, options = {}) => NanoBotz.copyNForward(jid, vM, forceForward, options)
             ...
             * @param {Object} options
            */
            m.quoted.cMod = (jid, text = '', sender = m.quoted.sender, options = {}) => NanoBotz.cMod(jid, vM, text, sender, options)

            /**
             * Delete quoted message
             */
            m.quoted.delete = () => NanoBotz.sendMessage(m.quoted.chat, { delete: vM.key })
        }
    }
    m.name = m.pushName || NanoBotz.getName(m.sender)
    if (m.msg && m.msg.url) m.download = (saveToFile = false) => NanoBotz.downloadM(m.msg, m.mtype.replace(/message/i, ''), saveToFile)
    /**
     ...
     * @param {Object} options
     */
    m.reply = (text, chatId, options) => NanoBotz.reply(chatId ? chatId : m.chat, text, m, options)

    /**
     * Copy this message
     */
    m.copy = () => exports.smsg(NanoBotz, M.fromObject(M.toObject(m)))
     ...
     * @param {Boolean} forceForward
     */
    m.forward = (jid = m.chat, forceForward = false) => NanoBotz.copyNForward(jid, m, forceForward, options)
     ...
     * @param {Object} options
     */
     m.reply = async (text, chatId, options) => NanoBotz.reply(chatId ? chatId : m.chat, text, m, options)
    ...
    return NanoBotz.relayMessage(m.chat, msg.message, {
      contextInfo: {
    ...
    m.copyNForward = (jid = m.chat, forceForward = true, options = {}) => NanoBotz.copyNForward(jid, m, forceForward, options)
     ...
     * @param {Object} options 
     */
    m.cMod = (jid, text = '', sender = m.sender, options = {}) => NanoBotz.cMod(jid, m, text, sender, options)

    /**
     * Delete this message
     */
    m.delete = () => NanoBotz.sendMessage(m.chat, { delete: m.key })

    try {
        if (m.msg && m.mtype == 'protocolMessage') NanoBotz.ev.emit('message.delete', m.msg.key)
    } catch (e) {
        console.error(e)
    }
    return m
